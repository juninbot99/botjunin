	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Eai dboas @${num.split('@')[0]}\nBem vindo ao grupo por favor não seja ghots*${mdata.subject}*`
				...
				teks = `Xauzinho ja foi tarde  @${num.split('@')[0]}ðŸ‘‹`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
                ...
                break
                   ...
                   break
					...
					break
                                  case 'registrar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('vocÃª jÃ¡ estÃ¡ registrado no bot ;)')
					if (args.length < 1) return reply(`ParÃ¢metro incorreto tente novamente com comando *registrar \nCommand : ${prefix} registrar nome|idade\nContoh : ${prefix} registrar junin|18`)
					var reg = body.slice(8)
					var jeneng = reg.split("|")[0];
					var umure = reg.split("|")[1];
						user.push(sender)
						fs.writeFileSync('./database/json/user.json', JSON.stringify(user))
						client.sendMessage(from, `\`\`\`O registro foi bem sucedido com RG: JUNINBOTRG768\`\`\`\n\n\`\`\`Dia ${date} ${time}\`\`\`\n\`\`\`[Nome]: ${jeneng}\`\`\`\n\`\`\`[NÃºmero]: wa.me/${sender.split("@")[0]}\`\`\`\n\`\`\`[Idade]: ${umure}\`\`\`\n\`\`\`Para usar o bot\`\`\`\n\`\`\`Por favor\`\`\`\n\`\`\`enviar ${prefix}help\`\`\`\n\`\`\`\nTotal de usuÃ¡rios ${user.length}\`\`\``, text, {quoted: mek})
					break
                                case 'welcome':
					if (!isGroup) return reply(mess.only.group)
                                        if (!isUser) return reply(mess.only.daftarB)
